...
public class RecommendController {
    ...
    */
    ...
    public ResponseEntity<List<RecommendPerfumesResponse>> findPerfumeByReview(@RequestHeader("Authorization")String token) throws IOException {
        List<RecommendPerfumesResponse> perfumeByClick = recommendService.findPerfumeByReview(token);
        return ResponseEntity.status(HttpStatus.OK).body(perfumeByClick);
    }
     ...
     *   설문 기반 추천
     */
    @GetMapping("/recommend/survey-reverse/{perfumeId}")
    public ResponseEntity<List<RecommendPerfumesResponse>> findPerfumeBySurveyReverse(@PathVariable Long perfumeId) throws IOException {
        List<RecommendPerfumesResponse> perfumeBySurveyReverse = recommendService.findPerfumeBySurveyReverse(perfumeId);
