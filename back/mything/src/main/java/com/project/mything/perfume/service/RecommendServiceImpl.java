...
public class RecommendServiceImpl implements RecommendService{
     ...
     *   리뷰 기반 추천
     */
    @Cacheable(key = "#mID", value = "recommendPerfumesResponse")
    @Override
    public List<RecommendPerfumesResponse> findPerfumeByReview(String token) throws IOException {
        Long mid = jwtService.getUserIdFromToken(token);
        List<Review> reviews = reviewRepository.findByMemberId(mid);
        if(reviews.size() == 0){
        ...
        Long perfumeId = reviews.get(0).getPerfume().getId();
     ...
     */
    @Override
    public List<RecommendPerfumesResponse> findPerfumeBySurveyReverse(Long perfumeId) throws IOException {
        String url = "https://j8b207.p.ssafy.io/bigdata/recommend/survey-reverse/" + perfumeId + "/";
        Response execute = springToDjango(url);
        List<RecommendPerfumesResponse> recommendPerfumesResponse = recommend(execute);
        return recommendPerfumesResponse;
    }
